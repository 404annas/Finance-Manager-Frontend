...
const Users = () => {
    ...
    const { data: usersData, isLoading, isError, error } = useQuery({ queryKey: ["users"], queryFn: fetchUsers })

    const { mutate: deleteUserMutate, isLoading: isDeletingUser } = useMutation({
        mutationFn: deleteUsers, onSuccess: (data) => {
    ...
    if (isError) {
        ...
        return <div className="p-6 text-center text-red-600 p-regular">Error: {errorMessage}</div>;
    }
                        ...
                        {invitedUsers.map((user) => (
                            <div key={user._id} className="flex items-center justify-between px-4 py-6 bg-[#F6F9FC] shadow rounded-lg">
                                ...
                                <button
                                    ...
                                    className={`p-medium flex items-center gap-2 p-2 rounded-md transition-all duration-300 ${isDeletingUser ? 'hover:cursor-not-allowed' : 'text-red-600 hover:text-red-700 underline cursor-pointer'}`}
                                >
                                    {isDeletingUser ? (
                                        <>
